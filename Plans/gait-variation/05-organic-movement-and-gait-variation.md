...
**Status:** Active
**Current next step:** Run the focused Slice 4B PlayMode regression filter `IdleVerticalBobTests|IdleSwayTests|OrganicGaitVariationTests|JumpTests|SprintJumpStabilityTests` now that the prefab overrides for idle bob and micro-step are in place.

Active slice checkpoint: `slice/05-4a-idle-sway` now contains the gated idle lateral sway force path in `LegAnimator.cs`, the `IdleSwayTests.cs` outcome suite, prefab overrides tuned to `_idleSwayForce: 33` and `_idleSwayFrequency: 1.0`, and a hardened `JumpTests` harness/tolerance for the previously order-sensitive landing-absorption check. Focused PlayMode verification via `IdleSwayTests|OrganicGaitVariationTests|LateralPlacementNoiseTests|JumpTests|SprintJumpStabilityTests` finished `31/31` green on 2026-03-24. Slice 4B is in progress on `slice/05-4b-idle-bob`; `BalanceController.SetIdleBobOffset`, the `LegAnimator` idle bob plus micro-step runtime path, the new `IdleVerticalBobTests` PlayMode suite, and prefab overrides for `_idleBobAmplitude: 0.018`, `_idleBobFrequency: 0.3`, `_microStepIdleDelay: 2.5`, `_microStepDriftThreshold: 0.01`, and `_microStepCooldown: 1.5` are in place. The first focused PlayMode run failed at compile because the new test tried to read `MicroStepCount` directly across assembly boundaries; that access has been switched to reflection. The next focused run then failed `IdleBob_WhenStandingStill_ProducesVerticalOscillation` and `MicroStep_WhenIdleLong_FiresAtLeastOnce`, so the idle gate was first latched through low forward-speed sway motion. That got the micro-step test green but regressed `SprintJump_TwoConsecutiveJumps_DoesNotFaceplant`, so the latch was tightened to self-generated lateral sway only and the bob amplitude was increased. The remaining bob red is now addressed by remapping the bob wave to a one-sided sine envelope that matches the additive lift-only height-maintenance path before the next rerun.
