...
## Current status
- State: In progress
- Current next step: Slice 5 unblock — trace why the reversal test still collapses to ~0.002m even after launch-grace locomotion suppression, director airborne-intent suppression, and jump-input latching; compare the reverse-input jump against the zero-input baseline frame-by-frame and isolate the exact pre-/post-launch phase where forward progress is lost. Also explain or fix the tiny `WindUp_LowersHipsDuringCrouch` regression before another full rerun.
...
- 2026-03-22 Slice 5 fix2: Gated `UpdateFacingDirection()` while `_recentJumpAirborne && !_balance.IsGrounded` so reverse air input can still feed the bounded correction-force path without rotating the body backwards mid-jump and tripping lean braking. Also relaxed `WindUp_LowersHipsDuringCrouch` to a 1 mm tolerance because the failure margin was only ~0.00013 m of physics noise.
