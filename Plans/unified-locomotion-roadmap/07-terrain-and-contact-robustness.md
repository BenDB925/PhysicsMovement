...
- State: C7.1 through C7.4b are complete; C7.5 remains split into agent-sized slices; Chapter 7 remains in progress.
- Current next step: C7.5a runtime metadata contract.
...
4. [ ] C7.4 Recovery on terrain:
    ...
    - [x] C7.4b Terrain recovery tuning:
       ...
       - 2026-03-14: Complete. Made catch-step adjustments surface-aware in `StepPlanner`: `ApplyCatchStepStrideAdjustment` now scales stride extension by a surface retention factor (`TerrainCatchStepStrideRetention=0.50`) that reduces reach on degraded surfaces (unreliable landing), and `ApplyCatchStepLateralAdjustment` adds extra widening (`TerrainCatchStepExtraWiden=0.04`) proportional to surface instability. This makes the compound interaction between catch-steps (C4.5/C6.5) and surface bracing (C7.3b) intentional instead of sequential-but-ignorant. Three new contract tests verify: catch-step on degraded terrain produces shorter stride than flat, catch-step on degraded terrain produces wider lateral than flat, and catch-step on worst terrain still exceeds default swing stride. Telemetry confirms: step-down descent uses more controlled recovery (maxProgress=5.23m, minAlignment=0.396) with zero fallen frames; slope and step-up outcomes unchanged; all Chapter 6 recovery suites green (HardSnap, Spin, StumbleStutter, CharacterState, LocomotionDirector: 41/42, only pre-existing GetUpReliability flake).
...
- `Tools/Run-UnityTests.ps1 -Platform EditMode -TestFilter "PhysicsDrivenMovement.Tests.EditMode.Character.LocomotionContractsTests"` -> `Passed, Total=87, Passed=87, Failed=0` (C7.4b: 3 new terrain-aware catch-step tests)
- `Tools/Run-UnityTests.ps1 -Platform PlayMode -TestFilter "PhysicsDrivenMovement.Tests.PlayMode.LegAnimatorTests;PhysicsDrivenMovement.Tests.PlayMode.GaitOutcomeTests;PhysicsDrivenMovement.Tests.PlayMode.Arena01BalanceStabilityTests"` -> `Passed, Total=75, Passed=72, Failed=0, Ignored=3` (C7.4b PlayMode regression check)
- `Tools/Run-UnityTests.ps1 -Platform PlayMode -TestFilter "PhysicsDrivenMovement.Tests.PlayMode.HardSnapRecoveryTests;PhysicsDrivenMovement.Tests.PlayMode.SpinRecoveryTests;PhysicsDrivenMovement.Tests.PlayMode.StumbleStutterRegressionTests;PhysicsDrivenMovement.Tests.PlayMode.GetUpReliabilityTests;PhysicsDrivenMovement.Tests.PlayMode.CharacterStateTests;PhysicsDrivenMovement.Tests.PlayMode.LocomotionDirectorTests"` -> `Passed, Total=42, Passed=41, Failed=1` (C7.4b Chapter 6 recovery regression check; 1 failure is pre-existing `GetUpReliabilityTests.AfterLeftDirectionalImpulse` flake)
- `Tools/Run-UnityTests.ps1 -Platform EditMode -TestFilter "PhysicsDrivenMovement.Tests.EditMode.Character.LocomotionContractsTests;PhysicsDrivenMovement.Tests.EditMode.Character.GroundSensorTests;PhysicsDrivenMovement.Tests.EditMode.Environment.TerrainScenarioSceneTests"` -> `Passed, Total=94, Passed=94, Failed=0` (C7.4b broader EditMode gate)
...
- 2026-03-14: Completed C7.4b. Made catch-step adjustments surface-aware in `StepPlanner` so recovery and bracing adjustments interact coherently on terrain. Two changes: (1) `ApplyCatchStepStrideAdjustment` now scales stride extension by `surfaceRetention = Lerp(0.50, 1.0, quality / 0.85)` — on maximally degraded surfaces only 50% of the catch-step stride extension applies, preventing aggressive reach onto unreliable landing surfaces; (2) `ApplyCatchStepLateralAdjustment` adds extra widening (`TerrainCatchStepExtraWiden=0.04 * instability`) beyond the base catch-step widen, broadening the support polygon on degraded footing. The compound interaction between catch-step (C4.5/C6.5) and bracing (C7.3b) is now intentional — catch-steps on slopes/edges are shorter but wider, while flat-ground catch-steps remain unchanged. Three new contract tests verify stride shortening, lateral widening, and that catch-step intent is not fully canceled on worst terrain. Telemetry: step-down recovery now uses more controlled steps (maxProgress=5.23m, minAlignment=0.396, zero fallen frames); slope (6.61m) and step-up (11.86m) essentially unchanged. Chapter 6 recovery suites all green (41/42, only pre-existing GetUpReliability flake). Verification: LocomotionContractsTests 87/87, broader EditMode 94/94, PlayMode LegAnimatorTests+GaitOutcomeTests+Arena01BalanceStabilityTests 72/0/3.
