...
| **Note** | This is a Unity Editor task — agent should open the prefab and add the component, or wire it through the relevant builder/editor script. If not automatable, leave as a manual checklist item. |
...
- 2026-03-17: Completed Step 11. Wired GettingUp to ProceduralStandUp in CharacterState: added serialized `_proceduralStandUp` reference, `BeginProceduralStandUp()` / `CleanUpProceduralStandUp()` lifecycle, `HandleStandUpCompleted` (→ Moving/Standing) and `HandleStandUpFailed` (→ re-enter Fallen with failure severity). GettingUp→Standing transition now gated on `!_proceduralStandUpActive` when procedural path is running. Old impulse path preserved as fallback for non-surrender falls or when component is absent. `_getUpTimeout` remains as safety net. EditMode 8/8 passed, PlayMode HardSnapRecoveryTests + BalanceControllerTests 15/15 green. Chapter 4 complete. Wired `ImpactKnockdownDetector` onto the root hips object for both `Assets/Prefabs/PlayerRagdoll.prefab` and the live runtime asset `Assets/Prefabs/PlayerRagdoll_Skinned.prefab` (the skinned prefab is a fully saved copy, not a variant). Serialized defaults match Chapter 2 and the detector now references the root hips `Rigidbody`, `BalanceController`, and `CharacterState`. Focused verification passed via EditMode `LocomotionDirectorEditModeTests` (8/8) and PlayMode `BalanceControllerTests` (10/10). A direct run of `PlayerRagdollPrefabPlayModeTests` now leaves `PlayerRagdollPrefab_FromBackFall_RecoversToStanding` red because its 95° back-fall setup overlaps the new surrender-era behavior; revisit that assertion during Step 12's test update pass rather than treating it as a Step 6 wiring regression.
