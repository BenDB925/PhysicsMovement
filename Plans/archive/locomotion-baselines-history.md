# Locomotion Baselines History Archive

This file preserves the detailed baseline snapshots and comparison prose moved out of `LOCOMOTION_BASELINES.md` during the documentation-slimming slice on 2026-03-18. Open the root doc first for the current baseline index, known active reds, and the artifact paths worth opening first.

## Archived sections

- Chapter 1 baseline snapshot, parity refresh, and completion verification
- Expected knockdown behavior detail from the comedic knockdown overhaul
- Chapter 8 expression-stack baseline and completion verification
...
- Full-suite ordering still causes cross-fixture contamination for spin and hard-snap tests despite per-fixture layer-collision matrix save or restore. The contamination vector is not in the matrix itself, which is properly saved and restored, but likely in residual physics-engine state such as cached broadphase pairs or contact caches that persists across test fixtures when many fixtures run in the same Unity process.
...
- **Surrender threshold:** Character MAY enter Fallen during extreme tilts (>80 deg) or when recovery stalls with angle above 50 deg for more than 0.8 s. Normal locomotion, moderate tilts, and standard turning are unaffected.
- **External impact knockdown:** Hits delivering >5 m/s effective delta-v, direction-weighted, trigger immediate surrender and Fallen. Hits between 2.5-5 m/s apply stagger torque but do not force Fallen. During GettingUp the knockdown threshold is reduced to 60 percent. During Fallen it drops to 40 percent for re-trigger.
- **Floor dwell:** 1.5-3.0 s scaled by knockdown severity, 0-1. Light stumble-surrender is about 1.5 s; full slam is about 3.0 s. Re-knockdown during dwell resets the timer with max(current, new) severity, capped at 1.5x max dwell. Non-surrender falls retain the original `_getUpDelay + _knockoutDuration` timing.
- **Procedural stand-up:** about 1.5-2.5 s via `ProceduralStandUp` with 4 phases: OrientProne, ArmPush, LegTuck, Stand. Each phase is physics-gated and can fail, returning to Fallen with a short re-dwell. After 3 failures the safety-net forced stand fires. Only active for surrender-path falls; legacy impulse fallback serves non-surrender falls.
- **Joint spring profile:** On surrender, all joint drive springs ramp to 25 percent of baseline over 0.15 s. Stand-up phases ramp arm and leg springs independently, arm 150 percent, leg 120-200 percent. `ResetSpringProfile` restores baseline on completion.
- **Upright torque ramp:** `ClearSurrender` restores upright, height, and stabilization scales via smooth `Ramp*` methods over 0.3-0.4 s, not instant snap, to prevent stand-up pop.
...
Regression baseline refresh after the full C8.1-C8.4b expression stack: pelvis tilt, torso twist, lateral sway, accel or decel lean, reversal weight shift, speed-reactive arm swing, arm brace during recovery, arm raise during airborne, expression amplitude caps, and expression kill-switch during Fallen or GettingUp.
...
- **PlayMode C8 verification gate (31 tests across 9 suites):**
  ...
  - `MovementQualityTests` 3/4 passed, with 1 known pre-existing failure.
...
- **LapCourseTests:** 1/2 passed, with 1 known pre-existing failure.
...
| Upper-leg peak rotation | 60.7 deg | 60.7 deg | 60.5 deg | Stable |
| Stride sync fraction | 0.0% | 0.0% | 0.0% | Stable |
| HardSnap 90 deg recovery frame | 220 | 159 | 56 | Improved |
| HardSnap 90 deg post-turn progress | 5.95 m | 7.14 m | 7.63 m | Improved |
| HardSnap 90 deg max stalled frames | 109 | 60 | 4 | Improved |
...
- `WalkStraight_NoFalls` — straight course does not complete at 600 frames, waypoint index 0. Character does not fall, with `maxConsecutiveFallen=0`, but fails to progress on the specific straight course. Pre-existing since Chapter 1.
...
- `LapCourseTests.CompleteLap_WithinTimeLimit_NoFalls` — lap completes, 23 or 24 gates and 0 falls, but exceeds the 4000-frame time limit at 5742 frames. Pre-existing since Chapter 1 completion verification.
...
- Terrain outcomes, step-down and slope-up, remain stable with zero fallen transitions.
...
Final verification gate after all C8 work packages, C8.1a-C8.5e: expression stack plus physically motivated jump, wind-up, launch, arm coordination, landing absorption, and jump test suite update.
...
- **PlayMode C8 verification gate (38 tests across 9 suites):**
  ...
  - `StumbleStutterRegressionTests` 4/4 passed.
  - `GetUpReliabilityTests` 3/3 passed.
  - `MovementQualityTests` 1/4 passed in full suite, 3 failures below; `3/4` passed in isolation with 1 known pre-existing failure.
- **Full suite failures (all pre-existing or order-sensitive):**
  ...
  - `WalkStraight_NoFalls` — known pre-existing, passes in isolation and fails in full suite due to test-order contamination.
  - `TurnAndWalk_CornerRecovery` — order-sensitive, passes in isolation and fails in full suite due to test-order contamination.
...
- MovementQualityTests performance matches the C8.4c baseline when run in isolation, 3/4 with the same single pre-existing failure. The additional failures in the full 9-suite gate are test-order contamination from residual physics-engine state, documented since Chapter 1 completion verification.
- The full C8 expression plus jump stack is stable: 119 EditMode plus 35 or 38 PlayMode, with all 3 failures pre-existing or order-sensitive.
- Chapter 8 exit criteria are satisfied: motion style is recognizable, jump shows visible crouch to spring to airborne to land absorb, and there is no regression in walk, turn, terrain, recovery, or balance outcomes.
