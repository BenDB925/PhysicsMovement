...
|--------------------|---------|-------------|
| `Character/` | Ragdoll body, balance, movement, state machine, combat | `RagdollSetup`, `BalanceController`, `GroundSensor`, `PlayerMovement`, `LegAnimator`, `ArmAnimator`, `CharacterState`, `HandGrabber`, `HitReceiver` |
...
| **3E3 - World-Space Leg Swing** | **Complete (2026-02-22)** | `_useWorldSpaceSwing` (bool, default true) added. Leg swing now computed relative to world-space movement direction (velocity proxy, XZ input fallback). `ApplyWorldSpaceJointTarget` converts world rotation to connected-body local frame. Knee bend uses same world-horizontal axis so knees never fold into ground regardless of torso pitch. 3 new PlayMode tests; 104/104 PlayMode + 9/9 EditMode passing. |
