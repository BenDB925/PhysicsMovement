...
namespace McpUnity.Unity
{
    ...
    public class McpUnityServer : IDisposable
    {
        ...
        private static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            switch (state)
            {
                case PlayModeStateChange.ExitingEditMode:
                    // Keep the bridge alive when domain reload is disabled so in-flight PlayMode
                    // test runs can survive the EditMode -> PlayMode transition.
                    if (_instance.IsListening && WillDomainReloadOnEnterPlayMode())
                    {
        ...
        private static bool WillDomainReloadOnEnterPlayMode()
        {
