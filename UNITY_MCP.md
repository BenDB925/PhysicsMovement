...
- Prefer `Tools/Run-UnityTests.ps1` for authoritative unattended verification, lock handling, retries, and fresh `TestResults/*.xml` artifacts. Use Unity MCP test execution only for quick in-editor confirmation or when the open editor state matters.
...
| Script compile or quick test feedback | `mcp_mcp-unity_recompile_scripts`, `mcp_mcp-unity_run_tests` | Good for fast in-editor confirmation. Batch scripts remain the source of truth for regression artifacts. |
...
3. If you need authoritative regression evidence or CI-like artifacts, use `Tools/Run-UnityTests.ps1`.
4. If you need a quick smoke check without leaving the open editor, use `mcp_mcp-unity_run_tests`.
